...
CORRECTED VERSION: Properly uses LeRobot's task handling API
...
def collect_episode_with_planning(
    ...
    episode_idx: int,
    use_different_language_per_skill: bool = False
):
    """
    Collect a single episode with planning annotations in LeRobot format.
    
    CORRECTED: Properly passes task to add_frame()
    ...
    Args:
        ...
        use_different_language_per_skill: Whether to use skill-specific language
    """
    print(f"\n{'='*60}")
    ...
    for skill_idx, skill in enumerate(skills):
        ...
        # Decide which task string to use for this frame
        if use_different_language_per_skill:
            # Use skill-specific language instruction
            frame_task = language_instruction
        else:
            # Use consistent episode-level task description
        ...
        while time.time() - skill_start_time < timeout:
            ...
            # NOTE: We pass task here for inference, but NOT in the dataset frame
            policy_obs = {
                "observation.images.top": obs_dict["observation.images.top"],
                "observation.state": obs_dict["observation.state"],
                "task": language_instruction  # Always use skill-specific for inference
            }
            ...
            # CRITICAL: Do NOT include task in frame_data dict!
            frame_data = {
                "observation.images.top": obs_dict["observation.images.top"],
                "observation.state": obs_dict["observation.state"],
                "action": action,
            ...
            # CORRECT API USAGE: Pass task as separate parameter
            dataset.add_frame(frame_data, task=frame_task)
            
            # Store skill metadata separately (not in dataset)
            skill_metadata = {
    ...
    # End episode - this processes all frames and handles task indexing
...
def main():
    ...
    # Language variation options
    parser.add_argument(
        "--use-skill-language",
        action="store_true",
        help="Use different language instruction for each skill (experimental)"
    )
    ...
    if args.autonomous:
        ...
        print("Teleoperation mode - use LeRobot's record command")
        print("Example:")
    ...
    print(f"Language mode: {'Skill-specific' if args.use_skill_language else 'Episode-consistent'}")
    ...
    try:
        for episode_idx in range(args.num_episodes):
            ...
            collect_episode_with_planning(
                ...
                episode_idx=episode_idx,
                use_different_language_per_skill=args.use_skill_language
            )
